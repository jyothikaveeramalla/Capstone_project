        ...
        if form.is_valid():
            try:
                with transaction.atomic():
                    ...
                    return redirect('artisan_products', artisan_id=artisan.id)
...
def edit_product_view(request, product_id):
    ...
    Access: Only the artisan who created the product or admins can edit
    """
    product = get_object_or_404(Product, id=product_id)
    
    # Verify ownership: only the creator or admins can edit
...
def delete_product_view(request, product_id):
    ...
    Access: Only the artisan who created the product or admins can delete
    Soft delete: Sets status to 'discontinued' instead of hard delete
    """
    product = get_object_or_404(Product, id=product_id)
    artisan = product.artisan
    
    # Verify ownership: only the creator or admins can delete
    if product.artisan.user != request.user and not request.user.is_staff:
...
def my_products_view(request):
    """
    View all products created by the current artisan (Artisan-only).
